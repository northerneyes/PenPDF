...
- 2026-09-16 · Owner: eraser flicker — erased strokes reappear for a frame, then vanish. Cause: the eraser commits to `drawing` asynchronously (off-thread stroke splitting), so even a one-turn-deferred pen-up render saw the pre-erase drawing. Fix: never render speculatively at pen-up — render at pen-up only if a change already arrived mid-gesture; otherwise render on `drawingDidChange` (pen up), with a 250 ms fallback to idle when nothing changes at all.
