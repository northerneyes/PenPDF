...
- 2026-09-18 · **"Ink jumps on Lock" solved by the live log.** `canvas.contentOffset` moved by exactly −86 pt (the nav bar's adjusted top inset) when Lock set `isScrollEnabled = false` on PDFKit's scroll view, and moved back on unlock; scroll view offset, canvas frame and page origin never changed. Cause: UIKit's navigation-bar scroll-edge logic re-selects a content scroll view when the registered one stops scrolling and picked our canvas. Two earlier guesses (content insets; re-sync on lock) were wrong because nothing in *our* geometry changed. Fix: `ScreenCanvasView` ignores any `contentOffset` write not made by `ScreenInkController`; the controller re-mirrors after lock/unlock. Lesson: with a symptom like this, get the numbers first — the log cost one deploy and answered in a minute.
