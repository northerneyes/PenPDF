...
- 2026-09-16 · Pinch mirroring (branch `exp/s5b-pinch-mirror`, nice-to-have, NOT merged): owner — "ink moves in the wrong direction" during the pinch. Approach (scale the canvas live like PDFKit scales its tiles) is sound; the anchor/position maths is wrong: with the canvas frame in scroll-content space, `layer.position = k·o0` plus `transform = scale(k)` does not match how UIScrollView repositions the zoomed view (it scales about the zoom anchor and adjusts contentOffset). Next attempt: compute the canvas transform from the DOCUMENT VIEW's actual transform/frame each frame (`scrollView.delegate?.viewForZooming?` → its `frame` relative to the scroll view at start vs now) rather than from zoomScale/contentOffset alone. Owner is happy with the hide-during-pinch behaviour on `main` (95 % of the pain gone).
